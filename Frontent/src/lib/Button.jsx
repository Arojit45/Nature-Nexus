...
const Button = ({ title, onClick }) => {
  return (
    <div className="relative group overflow-hidden rounded-xl border-2 border-black w-full max-w-xs sm:max-w-sm md:max-w-md h-10 sm:h-11 md:h-12">
      <button
        onClick={onClick}
        className="relative z-10 w-full h-full text-sm sm:text-base md:text-lg font-semibold text-black group-hover:text-white px-2 flex justify-center items-center transition duration-200 active:scale-95"
      >
        {title}
      </button>
      <span
